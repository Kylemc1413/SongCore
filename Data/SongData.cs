...
namespace SongCore.Data
{
    ...
    public class ExtraSongData
    {
        ...
        {
            string infoFilePath = Path.Combine(songPath, "info.dat");
            if (!File.Exists(infoFilePath)) return;
            try
            {
                LoadData(File.ReadAllText(infoFilePath));
            }
            catch (Exception ex)
            {
        ...
        public static ExtraSongData FromJson(string json)
        {
            ExtraSongData songData = new ExtraSongData();
            songData.LoadData(json);
            return songData;
        }

        private void LoadData(string infoText)
        {
